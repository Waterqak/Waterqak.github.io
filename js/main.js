    ...
    grid.innerHTML = list.map(p => {
        ...
        const playBtn = p.link
            ? `<a href="${p.link}" target="_blank" onclick="event.stopPropagation()" class="btn btn-outline mt-4 w-full justify-center" style="padding:8px;border-radius:8px;font-size:10px;letter-spacing:.1em">
                 <i data-lucide="gamepad-2" style="width:13px;height:13px"></i> PLAY GAME
               </a>`
            : '';
        ...
        const playingBadge = p.rbxPlaying != null && p.rbxPlaying > 0
            ? `<span style="position:absolute;top:8px;left:8px;background:rgba(46,232,154,0.12);border:1px solid rgba(46,232,154,0.3);padding:3px 8px;border-radius:4px;font-family:'JetBrains Mono',monospace;font-size:8.5px;color:var(--green);display:flex;align-items:center;gap:4px">
                 <span style="width:5px;height:5px;border-radius:50%;background:var(--green);animation:livePulse 2s ease-out infinite;flex-shrink:0"></span>
                 ${window.fmtRbxNum ? window.fmtRbxNum(p.rbxPlaying) : p.rbxPlaying} online
               </span>`
            : '';

        const favBadge = p.rbxFav != null
            ? `<span style="font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--gold);margin-top:6px;display:flex;align-items:center;gap:3px">
                 <svg style="width:9px;height:9px" fill="currentColor" viewBox="0 0 24 24"><path d="M12 21l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.18L12 21z"/></svg>
                 ${window.fmtRbxNum ? window.fmtRbxNum(p.rbxFav) : ''} favorites
               </span>`
            : '';
