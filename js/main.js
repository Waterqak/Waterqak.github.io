...
function toggleMute() {
    ...
    if (muted) { bgm.pause(); icon.setAttribute('data-lucide', 'volume-x'); }
    else        { bgm.play(); icon.setAttribute('data-lucide', 'volume-2'); }
    lucide.createIcons();
}
...
function initScrollProgress() {
    SITE.sections.forEach(s => {
        ...
        pg.addEventListener('scroll', () => {
            ...
            if (nav) nav.classList.toggle('scrolled', pg.scrollTop > 40);
        }, { passive: true });
    });
}
...
function runBoot() {
    ...
    let i = 0;
    function next() {
        if (i >= lines.length) { setTimeout(() => { const b = document.getElementById('boot-btn'); if (b) { b.style.opacity = '1'; b.style.transform = 'none'; } }, 350); return; }
        const l = lines[i];
        const d = document.createElement('div');
...
function initParticles() {
    ...
    const spawn  = () => {
        const n = Math.min(50, Math.floor(innerWidth / 24));
    ...
    resize(); spawn();
    window.addEventListener('mousemove', e => { m.x = e.x; m.y = e.y; }, { passive: true });
    ...
    (function draw() {
        ...
        pts.forEach((p, i) => {
            ...
            const dx = m.x - p.x, dy = m.y - p.y, md = Math.hypot(dx, dy);
            if (md < 90 && md > 0) { p.x -= dx/md*1.2; p.y -= dy/md*1.2; }
...
function showToast(msg, color) {
    ...
    t.style.cssText = [
        `position:fixed;bottom:${84+(_toastN-1)*76}px;right:22px;z-index:9998;`,
        'background:rgba(5,9,26,0.97);',
        `border:1px solid rgba(255,255,255,0.07);border-left:3px solid ${c};`,
        'color:var(--text);font-size:11px;font-family:\'JetBrains Mono\',monospace;',
        'padding:11px 16px;border-radius:10px;',
        'box-shadow:0 10px 35px rgba(0,0,0,0.7);max-width:300px;line-height:1.5;',
        'transform:translate3d(16px,0,0) scale(0.97);opacity:0;pointer-events:none;',
...
const FUNNY_TOASTS = [
    ...
    "Sensei, you've been here a while.",
    'Coffee consumed. Productivity +20%.',
    'Anti-exploit updated. Take that.',
];
    ...
    document.querySelectorAll('[data-ripple]').forEach(btn => {
        btn.addEventListener('click', e => {
            ...
            s.style.cssText = [
                ...
                'animation:ripple 0.55s linear;background:rgba(255,255,255,0.16);z-index:100;',
                `left:${e.clientX-r.left}px;top:${e.clientY-r.top}px;`,
                'width:80px;height:80px;margin:-40px 0 0 -40px;transform:scale(0);',
            ].join('');
...
function initCursorTrail() {
    ...
    (function loop() {
        dots.forEach((dot,i) => {
            ...
            dot.el.style.cssText = `position:fixed;pointer-events:none;z-index:9999;border-radius:50%;contain:strict;width:${sz}px;height:${sz}px;background:rgba(26,168,255,${al});transform:translate3d(${dot.x-sz/2}px,${dot.y-sz/2}px,0);`;
        });
...
function renderProjects() {
    ...
    grid.innerHTML = list.map(p => {
        ...
        const btn = p.link ? `<a href="${p.link}" target="_blank" onclick="event.stopPropagation()"
            class="btn btn-outline mt-4 w-full justify-center text-xs" style="padding:8px;border-radius:8px;font-size:10px;letter-spacing:.1em;">
            <i data-lucide="gamepad-2" style="width:13px;height:13px"></i> PLAY GAME</a>` : '';
        return `
        <div class="card group" onmouseenter="playHover && playHover()">
            ...
            <div style="height:186px;overflow:hidden;background:rgba(0,0,0,0.5);border-bottom:1px solid rgba(255,255,255,0.05);position:relative">
                ${_media(p)}
                <div style="position:absolute;inset:0;background:rgba(0,0,0,0);transition:background .3s" class="group-hover:bg-transparent"></div>
                <span style="position:absolute;bottom:8px;left:8px;background:rgba(0,0,0,0.75);border:1px solid rgba(255,255,255,0.1);padding:3px 8px;border-radius:4px;font-family:'JetBrains Mono',monospace;font-size:9px;color:#ccc">${p.tags[0] || ''}</span>
            </div>
            <div style="padding:20px">
                <div class="flex justify-between items-start mb-2">
                    <h3 class="display font-black text-white text-lg leading-none group-hover:text-[${s.text}] transition-colors">${p.title}</h3>
...
function calcBudget() {
    ...
    if (sp) sp.textContent = cur !== 'USD'
        ? `≈ $${Math.ceil(total*0.0035)} USD`
        : `≈ ${Math.ceil(total).toLocaleString()} R$`;
}
...
function _stars(n, size) {
    return Array.from({length:5}, (_,i) =>
        `<svg style="width:${size||14}px;height:${size||14}px;display:inline-block" fill="${i<n?'#FFA800':'none'}" stroke="#FFA800" stroke-width="1.5" viewBox="0 0 24 24">
        ...
        <div style="padding:20px;padding-top:24px">
            <div class="flex items-start justify-between gap-3 mb-3">
                <div class="flex items-center gap-3">
                    <div style="width:38px;height:38px;border-radius:50%;background:rgba(255,168,0,0.1);border:1px solid rgba(255,168,0,0.3);display:flex;align-items:center;justify-content:center;font-family:'Rajdhani',sans-serif;font-weight:900;font-size:16px;color:var(--gold);flex-shrink:0">
...
function _buildStars() {
    ...
    sel.innerHTML = Array.from({length:5},(_,i)=>`
        <span class="star-pick" data-v="${i+1}" style="font-size:26px;cursor:pointer;filter:${i<5?'drop-shadow(0 0 6px rgba(255,168,0,0.5))':'none'};transition:transform .15s">
            <svg style="width:28px;height:28px;transition:all .15s" fill="${i<5?'#FFA800':'none'}" stroke="#FFA800" stroke-width="1.5" viewBox="0 0 24 24">
                <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"/>
            </svg>
        </span>`
    ).join('');
    sel.querySelectorAll('.star-pick').forEach(s => {
        ...
        s.addEventListener('mouseenter', () => _syncStars(v));
        s.addEventListener('mouseleave', () => _syncStars(_starRating));
...
function _syncStars(n) {
    document.querySelectorAll('.star-pick').forEach((s,i) => {
        ...
        if (svg) svg.setAttribute('fill', i<n?'#FFA800':'none');
...
function unlockAch(id) {
    ...
    card.style.cssText = [
        `position:fixed;bottom:${88+(n%3)*88}px;left:22px;z-index:9997;width:280px;`,
        'background:rgba(5,9,26,.98);border:1px solid rgba(255,184,58,.35);border-left:3px solid var(--gold);',
    ...
    card.innerHTML = `
        <div style="font-size:7.5px;letter-spacing:.14em;color:var(--gold);margin-bottom:5px">★ ACHIEVEMENT UNLOCKED</div>
        <div style="display:flex;align-items:center;gap:9px">
            <span style="font-size:24px">${a.icon}</span>
            <div><div style="color:#fff;font-weight:900;font-size:10.5px;letter-spacing:.06em">${a.title}</div>
            <div style="color:var(--dim);font-size:8.5px;margin-top:2px">${a.desc}</div></div>
        </div>`;
...
function _buildAchList() {
    return Object.keys(ACHS).map(id => {
        ...
        return `<div style="display:flex;align-items:center;gap:9px;padding:7px;border-radius:8px;margin-bottom:3px;background:${ok?'rgba(255,184,58,.05)':'rgba(255,255,255,.015)'};border:1px solid ${ok?'rgba(255,184,58,.2)':'rgba(255,255,255,.04)'}">
            <span style="font-size:18px;flex-shrink:0;${ok?'':'filter:grayscale(1);opacity:.2'}">${a.icon}</span>
            <div style="min-width:0">
                <div style="font-size:9.5px;font-weight:900;letter-spacing:.05em;font-family:'Rajdhani',sans-serif;color:${ok?'#fff':'var(--muted)'}">${a.title}</div>
                <div style="font-size:7.5px;font-family:'JetBrains Mono',monospace;color:${ok?'var(--dim)':'var(--muted)'}margin-top:1px">${ok?a.desc:'???'}</div>
            </div>${ok?'<span style="margin-left:auto;font-size:8px;color:var(--gold);flex-shrink:0">✓</span>':''}
        </div>`;
    }).join('');
}
...
function triggerTransmission() {
    ...
    card.id = id;
    card.style.cssText=[
        'position:fixed;bottom:84px;right:24px;z-index:9992;width:300px;',
        'background:rgba(5,9,26,.98);border:1px solid rgba(255,255,255,.07);',
        `border-top:2px solid ${c};border-radius:12px;overflow:hidden;`,
    ...
    card.innerHTML=`
        <div style="padding:9px 13px;border-bottom:1px solid rgba(255,255,255,.05);background:rgba(26,168,255,.02);display:flex;align-items:center;justify-content:space-between">
            <span style="font-family:'Rajdhani',sans-serif;font-weight:900;font-size:11px;letter-spacing:.1em;color:#fff">📡 TRANSMISSION</span>
            <div style="display:flex;align-items:center;gap:5px">
                <span style="font-size:7px;padding:2px 6px;border-radius:3px;border:1px solid ${c};color:${c}">${t.pri}</span>
                <button onclick="document.getElementById('${id}').remove()" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:14px;padding:0 2px;transition:color .15s" onmouseover="this.style.color='#fff'" onmouseout="this.style.color='var(--muted)'">×</button>
            </div>
        </div>
        <div style="padding:13px">
            <div style="font-size:7.5px;letter-spacing:.1em;color:${c};margin-bottom:8px">FROM: ${t.from} // SCHALE.DB</div>
            <p style="font-size:11px;color:var(--text);line-height:1.65;font-family:'Nunito',sans-serif;margin-bottom:10px">"${t.msg}"</p>
            <div style="display:flex;justify-content:space-between;align-items:center">
                <span style="font-size:7.5px;color:var(--muted)">${new Date().toLocaleTimeString()}</span>
                <button onclick="document.getElementById('${id}').remove()" style="font-size:8.5px;font-weight:700;padding:4px 12px;border-radius:5px;cursor:pointer;background:${c};color:#000;border:none;font-family:'JetBrains Mono',monospace">ACK</button>
            </div>
        </div>`;
    document.body.appendChild(card);
    requestAnimationFrame(()=>requestAnimationFrame(()=>{card.style.transform='translateX(0)';card.style.opacity='1';}));
    setTimeout(()=>{card.style.transform='translateX(330px)';card.style.opacity='0';setTimeout(()=>{const el=document.getElementById(id);if(el)el.remove();},440);},9000);
    playClick(660,0.08);
}
...
function initCLI() {
    ...
    const cmds = {
        help: ()=>[`<span style="${B}">Available commands:</span>`,
            `  <span style="${G}">about skills projects code estimator reviews contact</span>`,
            `  <span style="${G}">date whoami status neofetch coffee uwu hack sudo</span>`,
            `  <span style="${G}">git blame  ls  ping  clear  touch grass</span>`,
            `  <span style="${M}">(secrets hidden in the void)</span>`].join('<br>'),
        ...
        neofetch:  ()=>[`<span style="${B}">WATER</span>@<span style="${B}">kivotos</span>`,
            '  OS: KivotOS x64 · Host: SCHALE.DB v5',
            '  Shell: bash (certified bad decisions)',
            '  CPU: Galaxy Brain (2 cores, 0 free)',
            '  RAM: 16GB (14.9GB used by browser)',
            '  Coffee: <span style="color:var(--red)">CRITICAL LOW</span>',
            '  Bugs: 0 (official count)',
            `  <span style="color:var(--red)">●</span><span style="color:var(--gold)">●</span><span style="color:var(--green)">●</span><span style="color:var(--accent)">●</span><span style="color:var(--purple)">●</span>`].join('<br>'),
    ...
    logo.addEventListener('click', ()=>{
        ...
        if (n>=7) {
            ...
            const iv=setInterval(()=>{
                ...
                if(i>14){clearInterval(iv);document.documentElement.style.setProperty('--accent','#1AA8FF');document.documentElement.style.setProperty('--accent2','#5CCFFF');document.documentElement.style.setProperty('--glow','rgba(26,168,255,0.28)');}
            },100);
        }
    });
}
